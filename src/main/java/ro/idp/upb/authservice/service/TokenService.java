...
public class TokenService {
    ...
    public Optional<Token> findByToken(String token, TokenType tokenType) {
        ...
        if (!response.getStatusCode().is2xxSuccessful()) {
            ...
            Token tokenEntity = Token.builder()
                    ...
                    .build();
            Token associatedTokenEntity = Token.builder()
    ...
    public ResponseEntity<?> saveUserTokens(User user, String jwtToken, String refreshToken) {
        PostTokensDto dto = PostTokensDto.builder()
        ...
        String url = UrlBuilder.replacePlaceholdersInString(
                "${io-service-url}${tokens-endpoint}",
                params);
        String urlTemplate = UriComponentsBuilder.fromHttpUrl(
                url
                )
    ...
    public boolean isRefreshToken(String refreshToken) {
        ...
        String url = UrlBuilder.replacePlaceholdersInString(
                "${io-service-url}${tokens-endpoint}${is-refresh-token}/${token}",
                params);
